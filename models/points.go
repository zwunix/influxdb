...
// ParsePointsV1 parses points in buf but does not rewrite the measurement/field.
func ParsePointsV1(buf, mm []byte) ([]Point, error) {
	return ParsePointsWithPrecisionV1(buf, mm, time.Now().UTC(), "n")
}
...
// ParsePointsStringV1 is identical to ParsePointsV1 but accepts a string.
func ParsePointsStringV1(buf, mm string) ([]Point, error) {
	return ParsePointsV1([]byte(buf), []byte(mm))
}
